...
#[test]
fn server_fixture_preserves_lifecycle_callbacks() {
    let directory = tempfile::tempdir().expect("temporary directory");
    let context = BenchmarkContext::new(directory.path().join("peryx"), directory.path().join("report.toml"));
    let fixture = server("fixture");
    let base = (fixture.base_url)(8080);
    assert_eq!(
        (
            base.as_str(),
            (fixture.probe)(&base),
            (fixture.command.expect("fixture command"))(&context, 8080, directory.path()).get_program(),
        ),
        (
            "http://127.0.0.1:8080/",
...
fn public_report_types_preserve_serialized_fields() {
    ...
    assert_eq!(toml::from_str::<Row>(&toml::to_string(&row).unwrap()).unwrap(), row);
}
