...
Configure peryx as a container registry, start it, and pull an image through a cached proxy of
...
peryx removes one repository link for OCI blob deletion and leaves the payload in the content store because another
repository or ecosystem may reference the same digest. After you delete the image, inspect unreferenced bytes before
removing them:
...
The first command prints a dry-run plan. The second unlinks blobs that no registered blob-reference provider reports.

## Next steps

- [Pull a Docker Hub official image](hub-official-images.md): pull `ubuntu` through the cache by its short name, the way
  you would against Docker Hub.
