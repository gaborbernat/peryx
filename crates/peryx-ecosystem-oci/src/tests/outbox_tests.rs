//! The transactional outbox: each authoritative hosted mutation records one typed operation in the
//! same driver transaction that writes its rows, and only when journaling is enabled.
...
fn test_publish_manifest_records_the_reference_it_named(#[case] reference: Reference, #[case] tag: Option<String>) {
    ...
    quota::publish_manifest(&meta, "store", "app", "sha256:abc", &manifest(), &reference, None, true).unwrap();
