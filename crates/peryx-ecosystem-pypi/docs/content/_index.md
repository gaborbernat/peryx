...
coordination for the process.

| Python term           | peryx concept | Meaning                                             |
| --------------------- | ------------- | --------------------------------------------------- |
| index (`--index-url`) | index         | Client endpoint; a cached index fronts one upstream |
| project or package    | project       | One distribution name, such as `requests`           |
| release or version    | version       | One released project version                        |
| distribution          | artifact      | A wheel or source-distribution file                 |
| file                  | file          | One content-addressed distribution file             |
| publish or upload     | upload        | Add a distribution to a hosted index                |
| install or download   | download      | Fetch a distribution through peryx                  |
| pull-through mirror   | cached index  | Read-through proxy for one upstream index           |
...
Publishing can use a [hosted layer with an upload token](@/guides/publish.md) or exchange a
[GitHub Actions or GitLab CI identity](@/guides/trusted-publishing.md) for a short-lived token. The CI flow scopes that
token to one repository route and the configured project globs.
...
The PyPI implementation labels searchable entities as packages and opens an index card on its project list. A project
page shows the long description, summary, install command, versions, dependencies, project links, classifiers, and
files. Release groups follow PEP 440 order. Files that do not map to one declared release remain visible under **Legacy
or unassociated files**.
...
[yank and delete packages](@/guides/remove.md).

<figure class="screen">
  <img class="screen-light" src="/screens/project-light.png"
       alt="A project page with description, metadata, releases, and distribution files" loading="lazy">
  <img class="screen-dark" src="/screens/project-dark.png"
       alt="A project page with description, metadata, releases, and distribution files" loading="lazy">
  <figcaption>A project page with description, metadata, releases, and distribution files</figcaption>
</figure>

## Related

- How peryx compares to devpi, proxpi, pypiserver, and pypicloud: [PyPI performance](@/performance.md)
...
- Block compromised distributions without deleting evidence: [digest revocations](@/revoked-content.md)
