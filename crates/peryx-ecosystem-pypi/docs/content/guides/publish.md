...
description = "Upload distributions with twine or uv, including wheels from older tooling and clients that declare one digest."
weight = 5
...
[matches the filename by normalized name and version](@/reference/uploads.md#wheel-dist-info-matching), with `METADATA`,
...
publishes neither the file nor its provenance. The [upload rules](@/reference/uploads.md#attestations) list each check
and limit.
...
- Accepted upload formats and validation: [upload behavior](@/uploads.md)
- Walk a legacy wheel and an MD5-only upload end to end:
  [publish and manage a release](@/tutorials/publish-and-manage.md)
