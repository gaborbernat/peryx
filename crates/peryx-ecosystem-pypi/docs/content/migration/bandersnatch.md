...
populated on first use at [no install-time penalty](@/core/operations/performance.md) or ahead of time with
`peryx mirror sync`, serves it itself (no [nginx](https://nginx.org/) layer, no name-normalization rewrite rules), stays
as fresh as upstream's `Cache-Control`, and hosts your private packages besides. bandersnatch's filter plugins narrow
the terabytes; requirements-based sync removes the guessing when clients already install from locks or requirements
files.
