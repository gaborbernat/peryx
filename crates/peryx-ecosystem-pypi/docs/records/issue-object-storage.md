...
peryx stores blobs on the local filesystem under `peryx-data/blobs`. That is simple and fast for one node, but it ties
...
- Add S3-compatible config:
  ...
  - retry limit
  - timeout
- Use AWS default credential resolution instead of storing access keys in peryx config.
