...
fn requests_if_server_ran(server_ran: bool, requests: Vec<u64>) -> Option<Vec<u64>> {
    server_ran.then_some(requests)
}
...
async fn load_round(
    ...
    http: &reqwest::Client,
    windows: &LoadWindows,
) -> anyhow::Result<Vec<SwarmResult>> {
    warm_pages(index_url, http).await?;
    let mut outcomes = Vec::with_capacity(users.len());
    for &count in users {
        outcomes.push(swarm(index_url, count, windows).await?);
    }
    Ok(outcomes)
}
...
#[cfg(test)]
#[path = "../../../tests/unit/bench/workloads/load.rs"]
mod tests;
