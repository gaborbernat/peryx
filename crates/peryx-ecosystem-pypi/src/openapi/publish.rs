...
pub(super) fn yank() -> OperationBuilder {
    ...
    .parameter(parameter(
        "reason",
        ParameterIn::Query,
        "Optional PEP 592 yank reason preserved in Simple API output",
    ...
    OperationBuilder::new()
        ...
        .parameter(
            parameter(
                "from",
                ParameterIn::Query,
                "Source route whose local upload layer contains the release",
                json!("staging"),
            )
...
fn version_removal_operation(summary: &str, description: &str, example: &str) -> OperationBuilder {
    removal_operation(summary, description, example).parameter(version_param())
}
