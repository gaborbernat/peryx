//! Hosted PEP 740 upload binding, provenance serving, visibility, and policy tests.
...
async fn test_hosted_provenance_with_a_missing_blob_is_not_found() {
    ...
    let provenance = harness.state.meta.get_provenance(digest.as_str()).unwrap().unwrap().0;
    assert!(
        harness
            .state
            .blobs
