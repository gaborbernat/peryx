//! Digest revocation across `PyPI` discovery, artifact, sibling, and inspection routes.
...
async fn test_revoked_digest_is_removed_from_each_project_representation() {
    ...
    for (uri, accept) in [
        ...
        ("/+ui/project?index=pypi&project=flask", "application/json"),
    ] {
...
async fn test_revoked_digest_is_not_read_through_any_byte_route() {
    ...
    let metadata_digest = h.state.blobs.put_bytes(metadata).await.unwrap();
    h.state
        .meta
        .put_metadata(
            artifact.as_str(),
    ...
    let provenance_digest = h.state.blobs.put_bytes(provenance).await.unwrap();
    h.state
        .meta
