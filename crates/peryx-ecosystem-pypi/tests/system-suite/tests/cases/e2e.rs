...
use peryx_test_support::{Node, ProcessHarness, ProcessLimit};
use rstest::rstest;
...
impl Peryx {
    ...
    fn start_against_with_overlay_policy(upstream_url: &str, policy_toml: &str) -> Self {
        ...
        let node = ProcessHarness::new(env!("CARGO_BIN_EXE_peryx-pypi-system-server"))
...
fn e2e_client_runner_reports_unexpected_status(
    ...
    let mut command = Command::new(env!("CARGO_BIN_EXE_peryx-pypi-system-server"));
