...
pub fn publish_payload(node: &Node, payload: &[u8]) -> Result<(u16, String), reqwest::Error> {
    let form = reqwest::blocking::multipart::Form::new()
        .text(":action", "file_upload")
        .text("name", "veloxdemo")
        .text("version", "1.0.0")
        .text("filetype", "bdist_wheel")
        .text("sha256_digest", Digest::of(payload).as_str().to_owned())
        .part(
            "content",
            reqwest::blocking::multipart::Part::bytes(payload.to_vec()).file_name(WHEEL_FILENAME),
        );
    let response = node
