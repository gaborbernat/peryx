...
fn read_acl(projects: &str) -> IndexAcl {
    IndexAcl {
        ...
        tokens: vec![NamedToken {
            ...
            grants: vec![Grant {
                resources: vec![Glob::new(projects)],
...
async fn state_with_hosted_acl(acl: IndexAcl) -> Arc<AppState> {
    ...
    Arc::get_mut(&mut state.serving).unwrap().indexes[1].acl = acl;
    Arc::new(state)
}
...
async fn test_changelog_uses_the_listing_rate_limit() {
    ...
    let Harness { state, .. } = h;
    let mut state = Arc::try_unwrap(state).ok().unwrap();
    Arc::get_mut(&mut state.serving).unwrap().rate_limits = RateLimiter::new(RateLimitConfig {
