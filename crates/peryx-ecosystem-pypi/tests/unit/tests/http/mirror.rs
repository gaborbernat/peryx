...
async fn test_mirror_json_already_local_record_round_trips() {
    ...
    h.state.serving.meta.put_index("pypi/flask", &record).unwrap();
...
async fn test_mirror_detail_serves_stale_json_then_revalidates_in_background() {
    ...
    let (status, ..) = tokio::time::timeout(
        ...
        get(&h.state, "/pypi/flask/json", None),
    )
    .await
...
fn corrupt_cached_record(h: &Harness, key: &str) {
    ...
    h.state.serving.meta.put_index(key, &record).unwrap();
    ...
    let raw_key = h
        ...
        .into_iter()
        .find(|candidate| {
            h.state.serving.meta.get_driver_value(candidate).unwrap().as_deref() == Some(encoded.as_slice())
        })
    ...
    h.state
        ...
        .unwrap();
    assert!(
        h.state.serving.meta.get_index(key).is_err(),
        "the seeded record is undecodable"
    );
}
#[tokio::test]
async fn test_mirror_json_refetches_when_the_cached_record_is_undecodable() {
    ...
    assert!(body.contains(&format!("/pypi/files/{}/flask-1.0-py3-none-any.whl", digest.as_str())));
    assert!(
        h.state.serving.meta.get_index("pypi/flask").unwrap().is_some(),
