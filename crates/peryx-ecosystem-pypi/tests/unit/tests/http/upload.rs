...
use peryx_driver::serving::{
    BlobReferenceDriver as _, CacheDriver as _, FsckDriver as _, ImportDriver as _, PolicyDryRunDriver as _,
};
...
#[tokio::test]
async fn test_upload_accepts_existing_content_under_a_new_intent() {
    ...
    assert_eq!(
        post_upload_response(&source.state, "/hosted/", Some(&upload_auth()), &content_type, body,)
            .await
            .0,
        StatusCode::OK
    );
    ...
    target
        ...
        .serving
        .meta
        .put_upload("hosted", "peryxpkg", filename, &record)
        .unwrap();
    let (content_type, body) = multipart_body(&fields, Some((filename, &wheel)));
    let (status, body) =
        post_upload_response(&target.state, "/hosted/", Some(&upload_auth()), &content_type, body).await;

    assert_eq!((status, body.as_str()), (StatusCode::OK, "upload accepted"));
    assert_eq!(target.state.serving.meta.count_staged_intents().unwrap(), 1);
}
...
async fn test_upload_durably_stages_one_ingress_intent_and_deduplicates_a_resend() {
    ...
    assert_eq!(h.state.serving.meta.count_staged_intents().unwrap(), 1);
...
async fn test_policy_rejects_chunked_upload_over_limit(#[case] hosted_limit: u64, #[case] virtual_limit: u64) {
    let hosted_policy = policy(|neutral, _pypi| {
        neutral.max_artifact_size_bytes = Some(hosted_limit);
    });
    let virtual_policy = policy(|neutral, _pypi| {
        neutral.max_artifact_size_bytes = Some(virtual_limit);
    });
...
async fn test_upload_body_failure_removes_the_stage() {
    ...
    sender
        .send((Ok(Bytes::copy_from_slice(&body[..split])), Some(consumed_sender)))
        .await
        .unwrap();
    ...
    let request = tokio::spawn(async move {
        post_upload_body_with_headers_response(
            ...
            Body::from_stream(futures_util::stream::unfold(
                ...
                |(mut receiver, consumed_sender)| async {
                    if let Some(consumed_sender) = consumed_sender {
                        let _ = consumed_sender.send(());
                    }
                    receiver
                        .recv()
                        .await
                        .map(|(item, consumed_sender)| (item, (receiver, consumed_sender)))
                },
            )),
        )
        .await
    });
    tokio::time::timeout(std::time::Duration::from_secs(5), consumed_receiver)
        .await
        .unwrap()
        .unwrap();
    sender.send((Err(std::io::Error::other("reset")), None)).await.unwrap();
...
async fn test_policy_accepts_upload_at_size_limit() {
    ...
    let overlay_policy = policy(|neutral, _pypi| {
        neutral.max_artifact_size_bytes = Some(wheel.len() as u64);
    });
...
async fn test_concurrent_different_bytes_uploads_leave_one_deterministic_winner() {
    ...
    let winning_digests: Vec<String> = [(status_first, &first), (status_second, &second)]
        .into_iter()
        .filter(|(status, _)| *status == StatusCode::OK)
        .map(|(_, bytes)| Digest::of(bytes).as_str().to_owned())
        .collect();
    assert_eq!(
        winning_digests,
        std::slice::from_ref(&served_sha),
...
async fn test_pypi_maintenance_scans_walk_real_records() {
    ...
    let report = crate::serving::PypiServing
        .purge_resource(meta, "pypi", "flask", true)
        .unwrap();
...
async fn test_pypi_policy_dry_run_writes_a_denial() {
    let h = harness_with_policies(
        true,
        true,
        policy(|neutral, _pypi| neutral.block_resources = vec!["flask".to_owned()]),
        Policy::default(),
        Policy::default(),
    )
    .await;
    ...
    let mut out = Vec::new();
    crate::serving::PypiServing
        .policy_dry_run(&h.state.serving.meta, &h.state.serving.indexes, None, None, &mut out)
        .unwrap();
    assert!(
...
fn state_with_trusted_publishing(enabled: bool) -> (tempfile::TempDir, Arc<AppState>, Signer) {
    ...
    state.set_token_realm(signer.clone(), 300).unwrap();
    if enabled {
        crate::trusted_publishing::install(
...
async fn test_oci_realm_token_is_not_accepted_for_pypi_upload() {
    ...
    let token = signer.mint(
        ...
        &[Grant {
            resources: vec![Glob::new("hosted/peryxpkg")],
            actions: BTreeSet::from([Action::Write]),
        }],
        1,
        i64::MAX / 2,
...
async fn test_a_stalled_upload_body_reports_a_timeout() {
    ...
    assert!(body.contains("sent nothing for"), "{body}");
}

/// A client sending just often enough to keep the gap bound satisfied still has to get somewhere, and
/// the floor ends it when it does not. That is the same client-side condition as a stall, so it earns
/// the same answer rather than falling through to the malformed-form reply.
#[tokio::test(start_paused = true)]
async fn test_a_trickling_upload_body_reports_a_timeout() {
    let h = harness().await;
    let (content_type, body) = multipart_body(&[], Some(("peryxpkg-1.0-py3-none-any.whl", b"ab")));
    // A frame every twenty-nine seconds never goes quiet, and earns a fraction of a second each time.
    let dribble: Vec<Bytes> = body
        .chunks(body.len().div_ceil(3))
        .map(Bytes::copy_from_slice)
        .collect();
    let trickle = futures_util::StreamExt::then(futures_util::stream::iter(dribble), |frame| async move {
