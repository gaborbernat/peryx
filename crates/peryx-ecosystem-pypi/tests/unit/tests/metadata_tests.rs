    ...
    assert_eq!(
        (
            ...
            project.files[0]
                .lifecycle
                .as_ref()
                .and_then(|lifecycle| lifecycle.reasons.first())
                .map(String::as_str),
        ),
        (expected, reason),
    );
...
fn test_ui_project_from_detail_marks_a_release_its_publisher_yanked_whole(
    ...
    assert_eq!(
        ...
        [ReleaseView {
            ...
            lifecycle: yanked.then(|| LifecycleView {
...
fn test_ui_meta_exposes_contacts_in_a_fixed_order(#[case] headers: &str) {
    ...
    assert_eq!(
        ...
        [
            MetadataBlock::KeyValue {
                label: "Author".to_owned(),
                value: "Jane".to_owned(),
            },
            MetadataBlock::KeyValue {
                label: "Author Email".to_owned(),
                value: "jane@x.test".to_owned(),
            },
            MetadataBlock::KeyValue {
                label: "Maintainer".to_owned(),
                value: "Joe".to_owned(),
            },
            MetadataBlock::KeyValue {
                label: "Maintainer Email".to_owned(),
                value: "joe@x.test".to_owned(),
            },
        ]
    );
}
...
fn test_ui_meta_renders_the_description_and_chips() {
    ...
    assert!(
        meta.blocks
            .iter()
            .any(|block| matches!(block, MetadataBlock::Chips { label, .. } if label == "Keywords"))
    );
    assert!(
        meta.blocks
            .iter()
            .any(|block| matches!(block, MetadataBlock::Chips { label, .. } if label == "Dependencies"))
    );
}
...
fn test_file_matches_version_wheel_and_sdist() {
    ...
    assert!(file_matches_version("peryxpkg-1.0-py3-none-any.whl", "1.0.0"));
...
// CRLF descriptions must not promote body text into metadata fields.
#[rstest]
#[case::crlf_document(
    "Name: x\r\nVersion: 1\r\n\r\nDescription body.\r\nVersion: 2 in prose.\r\n",
    "Description body.\r\nVersion: 2 in prose.\r\n"
...
#[case::homepage_alias("HOMEPAGE", "Homepage")]
#[case::download_alias("DOWNLOAD", "Download")]
#[case::changelog_alias("CHANGELOG", "Changelog")]
#[case::changes_alias("Changes", "Changelog")]
#[case::whatsnew_alias("What's New", "Changelog")]
#[case::history_alias("History", "Changelog")]
#[case::release_notes_alias("Release-Notes", "Release Notes")]
...
fn test_ui_meta_normalizes_well_known_project_url_labels(#[case] label: &str, #[case] displayed: &str) {
    ...
    assert_eq!(
        crate::ui_meta(&text).unwrap().blocks,
        [MetadataBlock::Links {
            label: "Links".to_owned(),
            links: vec![(displayed.to_owned(), "https://example.test".to_owned())],
        }]
    );
}
...
fn test_parse_metadata_keeps_raw_project_url_labels() {
    // Metadata normalization must preserve the published version label.
    let doc = parse_metadata(
...
#[rstest]
#[case::public("foo.bar", Some("foo.bar"))]
#[case::marker_trimmed_and_dropped("foo ; private", None)]
#[case::empty("", None)]
fn test_ui_meta_shows_only_public_import_names(#[case] value: &str, #[case] expected: Option<&str>) {
...
fn test_ui_meta_separates_import_names_from_namespaces() {
    ...
    assert!(blocks.contains(&MetadataBlock::Chips {
        label: "Import Names".to_owned(),
