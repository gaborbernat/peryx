...
use peryx_policy::{
    ArtifactFacts, Policy, PolicyAction, PolicyConfig, PolicyDecisionRecorder, PolicyDecisionState, PolicyEvaluation,
};
...
use crate::policy::{
    ...
    REQUIRED_ATTESTATION_RULE, compile_capabilities,
};
...
#[test]
fn test_policy_settings_expose_default_fallback_and_proxy_metadata() {
    let config = black_box(PypiPolicyConfig {
        upstream_attestations: RemoteMetadataMode::Proxy,
        ..PypiPolicyConfig::default()
    });
    let policy = Policy::compile(&PolicyConfig::default(), crate::normalize_name)
        .with_capabilities(compile_capabilities(&config).unwrap());

    assert_eq!(
        (
            black_box(FallbackMode::Fallback).to_string(),
            policy.owner_setting("pypi.remote-metadata-mode"),
        ),
        ("fallback".to_owned(), Some("proxy"))
    );
}
...
fn test_apply_list_filters_project_rules() {
    let policy = policy(|neutral, _pypi| {
        neutral.block_resources = vec!["bad-pkg".to_owned()];
    });
...
fn test_apply_detail_rejects_project_size_over_limit() {
    let policy = policy(|neutral, _pypi| {
        neutral.max_resource_size_bytes = Some(10);
    });
...
fn test_check_project_denies_project_outside_allow_list() {
    let policy = policy(|_neutral, pypi| {
        pypi.allow_projects = vec!["flask".to_owned()];
    });

    let denial = policy.check_resource(PolicyAction::Serve, "django").unwrap_err();
...
fn test_preview_detail_reports_file_and_project_size_denials() {
    let policy = policy(|neutral, pypi| {
        ...
        neutral.max_resource_size_bytes = Some(5);
    });
...
fn test_protected_name_blocks_upstream_across_pep503_spellings() {
    let policy = policy(|neutral, _pypi| {
        neutral.protected_resources = vec!["Acme.Secrets".to_owned(), "acme_internal_*".to_owned()];
    });

    for spelling in ["acme-secrets", "Acme_Secrets", "acme.secrets", "acme-internal-db"] {
        assert!(
            policy
                .check_resource(PolicyAction::Cached, &crate::normalize_name(spelling))
                .is_err(),
            "{spelling} should not fall back upstream"
        );
    }
    assert_eq!(policy.check_resource(PolicyAction::Serve, "acme-secrets"), Ok(()));
}
...
fn aged_facts(upload_time: Option<i64>, now: Option<i64>) -> ArtifactFacts {
    ArtifactFacts {
        ...
        artifact: Some("demo-1.0-py3-none-any.whl".to_owned()),
        upload_time,
        now,
...
fn test_release_delay_verdict_by_age_and_clock() {
    ...
    let cases = [
        ...
        // Catalog paths have no serve-time clock for age evaluation.
        Case {
...
fn test_release_delay_clamps_an_out_of_range_age() {
    // Overflowing delays saturate instead of wrapping into an eligible time.
...
fn test_required_attestation_ignores_a_serve_fact_that_carries_no_attestations() {
    // Attestation requirements apply only at the upload boundary.
