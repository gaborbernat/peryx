...
// Reject malformed identities so distinct clients never share the peer bucket.
#[rstest::rstest]
...
async fn test_authenticated_principal_ignores_malformed_forwarded_header() {
    ...
    // Principal identity takes precedence over forwarded network identity.
...
async fn test_denials_are_logged_and_counted() {
    ...
    let event = events.iter().find(|event| field(event, "event") == Some("rate_limit"));
    assert!(event.is_some(), "{}", capture.text());
...
async fn test_token_denials_are_logged_as_token_clients() {
    ...
    let events = capture.security_events();
    let event = events.iter().find(|event| field(event, "client") == Some("token"));
    assert!(event.is_some(), "{}", capture.text());
    let event = event.unwrap();
...
async fn test_upstream_concurrency_cap_applies_backpressure() {
    ...
    Mock::given(method("GET"))
        .respond_with(BlockingResponder {
    ...
    let first = tokio::spawn(async move {
        ...
        request(&first_state, "/pypi/simple/flask/", &[("x-forwarded-for", "192.0.2.1")]).await
    });
    ...
    entered_receiver
        ...
        .expect("upstream request did not enter responder");
    ...
    .await
    .expect("upstream requests did not complete");
...
impl Respond for BlockingResponder {
    fn respond(&self, _request: &WiremockRequest) -> ResponseTemplate {
        if self.calls.fetch_add(1, Ordering::SeqCst) == 0 {
            ...
            self.release
                ...
                .unwrap()
                .recv_timeout(Duration::from_secs(2))
                .expect("test did not release upstream response");
        }
        self.response.clone()
    }
}
...
async fn test_upstream_acquire_times_out_when_saturated() {
    ...
    // The full wait horizon prevents immediate retries from re-saturating the limiter.
...
fn test_state_with_rate_limits_sets_limiter_and_upstream_cap() {
    ...
    assert!(state.serving.rate_limits.enabled());
...
fn test_state_with_search_path_uses_disabled_limiter() {
    ...
    assert!(!state.serving.rate_limits.enabled());
    assert!(state.serving.upstream_limits.snapshots().is_empty());
}
