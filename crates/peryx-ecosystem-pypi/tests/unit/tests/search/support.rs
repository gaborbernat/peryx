...
pub(super) use peryx_driver::state::{AppState, ServingState};
...
pub(super) fn put_uploaded_package(state: &ServingState, display: &str, normalized: &str, summary: &str) {
...
pub(super) fn put_uploaded_package_with_metadata(
    ...
    let uploaded = Uploaded {
        ...
        file: File {
            ...
            url: local_artifact_url("hosted", artifact_digest.as_str(), &filename),
...
pub(super) fn put_uploaded_file(state: &ServingState, normalized: &str, version: &str) -> String {
...
pub(super) fn trash_upload(state: &ServingState, normalized: &str, filename: &str) {
    let (_, bytes) = state
        .meta
...
pub(super) fn put_cached_package(
    ...
    let record = cached_index(&to_json(detail));
    let files = detail
        .files
        .iter()
        .filter_map(|file| {
