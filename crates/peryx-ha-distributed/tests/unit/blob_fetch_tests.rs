...
impl BlobTransport for Counting {
    async fn fetch_blob(&self, request: BlobRequest) -> Result<Vec<u8>, TransportError> {
        ...
        if self.started.fetch_add(1, Ordering::SeqCst) < self.first_wave_size {
...
async fn test_fetch_missing_runs_up_to_the_concurrency_bound_at_once() {
    ...
    let task = tokio::spawn(async move { fetch_missing(transport.as_ref(), &digests(4), nz(2)).await });
...
async fn test_fetch_missing_serializes_at_a_bound_of_one() {
    let max = Arc::new(AtomicUsize::new(0));
    let first_wave = Arc::new(tokio::sync::Barrier::new(2));
    let transport = Arc::new(Counting {
        in_flight: Arc::new(AtomicUsize::new(0)),
        max: Arc::clone(&max),
        started: AtomicUsize::new(0),
    ...
    let task = tokio::spawn(async move { fetch_missing(transport.as_ref(), &digests(4), nz(1)).await });
