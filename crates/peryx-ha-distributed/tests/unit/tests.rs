...
#[path = "placement_policy_contract_tests.rs"]
mod placement_policy_contract_tests;
...
#[tokio::test]
async fn test_primary_reports_a_missing_page() {
    let source = primary(Vec::new());

    let error = source.changes(7, 1).await.unwrap_err();

    assert_eq!(error.0, "no page after 7");
    assert_eq!(*source.requests.lock().unwrap(), vec![7]);
}
