...
impl Fixture {
    ...
    async fn request(
        ...
        self.raw(method, uri, credential, body, Some("application/json"), None)
            .await
    }
...
async fn test_every_mutating_route_challenges_an_anonymous_request() {
    ...
    assert_eq!(
        fixture
            .raw(Method::DELETE, &uri, None, None, None, Some("\"1\""))
            .await
            .0,
        StatusCode::UNAUTHORIZED
    );
...
async fn test_revocation_demands_a_well_formed_precondition() {
    ...
    let missing = fixture.raw(Method::DELETE, &uri, admin, None, None, None).await;
    assert_eq!(missing.0, StatusCode::PRECONDITION_REQUIRED);

    let malformed = fixture
        .raw(Method::DELETE, &uri, admin, None, None, Some("not-a-version"))
        .await;
