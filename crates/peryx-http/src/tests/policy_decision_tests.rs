...
async fn app_with_options(
    ...
    if let Some(state) = token_user_state {
        let user = users.create("__token__").unwrap();
        users.set_password(&user.id, USER_PASSWORD).await.unwrap();
    ...
    if !matches!(fault, StoreFault::None) {
        ...
        let table = match fault {
            StoreFault::Identity => "server_user",
            StoreFault::Authentication => "server_user_verifier",
            StoreFault::Authorization => "role_grant",
            StoreFault::Query => "policy_decision",
        ...
        document["decisions"]
            ...
            .map(|decision| {
                (
                    ...
                    decision["project"].as_str().unwrap(),
                )
            })
            .collect::<Vec<_>>(),
        expected
...
async fn test_policy_decisions_rejects_invalid_pagination(#[case] uri: &str, #[case] error: &str) {
    let (_dir, _meta, app) = app().await;
    let response = get(&app, uri, Some(("__token__", ADMIN_SECRET))).await;
