...
use peryx_search::{IndexerCtx, PackageDocument, PackageIndexer, SearchError};
...
impl PackageIndexer for StubIndexer {
...
impl Fixture {
    ...
    async fn with_fault(driver: StubDriver, fault: StoreFault) -> Self {
        ...
        state.register_ecosystem(Arc::new(driver), Arc::new(StubIndexer));
...
fn plan_body(repository: &str) -> serde_json::Value {
    serde_json::json!({"repository": repository, "expire": [{"selector": "project-prefix", "prefix": ""}]})
}
...
async fn test_plan_refuses_when_the_repository_is_at_its_concurrency_bound() {
    ...
    .await;
    let held: Vec<_> = (0..2)
        .map(|_| fixture.state.retention_gates.try_enter("hosted").unwrap())
        .collect();
