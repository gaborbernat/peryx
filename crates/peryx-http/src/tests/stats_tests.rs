...
async fn app_with_fault(fault: StoreFault) -> (tempfile::TempDir, Arc<AppState>) {
    ...
    let mut state = AppState::new(
        ...
        vec![Index {
            ...
            acl: IndexAcl::upload_token(UPLOAD_SECRET),
        }],
    );
...
async fn get(
    ...
    let mut request = Request::builder().uri("/+stats");
...
#[case::legacy_token(Some(("__token__", UPLOAD_SECRET)), StatusCode::UNAUTHORIZED)]
...
async fn test_stats_requires_operator_scope(#[case] credential: Option<(&str, &str)>, #[case] expected: StatusCode) {
    ...
    let (status, headers, _) = get(&state, credential).await;
...
async fn test_stats_fails_closed_on_store_faults(#[case] fault: StoreFault) {
    ...
    let (status, headers, body) = get(&state, Some(("Alice", USER_PASSWORD))).await;
