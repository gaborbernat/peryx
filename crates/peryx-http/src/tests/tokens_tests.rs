    ...
    async fn build(fault: StoreFault, preload: bool) -> Self {
        ...
        if fault != StoreFault::None {
            ...
            let table = match fault {
                ...
                StoreFault::Tokens => "scoped_token",
                StoreFault::TokenIndex => "scoped_token_verifier",
...
async fn test_create_rejects_malformed_requests() {
    ...
    // Invalid JSON, charset variant still recognized as JSON.
    assert_eq!(
        fixture
            .call(
                Method::POST,
    ...
    // Empty name, empty actions, past expiry.
    assert_eq!(
        fixture
