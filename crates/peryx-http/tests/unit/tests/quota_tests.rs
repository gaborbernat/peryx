...
/// Use the index name to exercise route-to-index quota lookup.
...
async fn test_summary_reports_counters_limits_and_paginates() {
    ...
    assert_eq!(private["resources"]["committed"], 1);
    assert_eq!(private["resources"]["reserved"], 1);
    assert_eq!(private["resources"]["remaining"], 3);
    assert_eq!(private["artifact_bytes"]["limit"], serde_json::Value::Null);
    assert_eq!(private["artifact_bytes"]["remaining"], serde_json::Value::Null);
    assert_eq!(private["limits"]["max_accounted_bytes"], 10_000);
    assert_eq!(private["limits"]["max_artifact_bytes"], 1024);
...
async fn test_detail_reports_one_repository_for_a_reader() {
    ...
    assert_eq!(body["limits"]["max_groups_per_resource"], serde_json::Value::Null);
}
...
#[case::scoped_credential(Some(("external", READER_SECRET)), StatusCode::FORBIDDEN)]
