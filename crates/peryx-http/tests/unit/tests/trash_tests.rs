...
use peryx_driver::serving::{EcosystemDriver, TrashDriver};
...
async fn test_trash_list_and_record_render_driver_records() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert_eq!(page["trash"][0]["resource"], "artifact-a");
...
#[case::write_token("/+trash?repository=private", Some(("external", ADMIN_SECRET)), StatusCode::OK)]
#[case::token_without_repository("/+trash", Some(("external", ADMIN_SECRET)), StatusCode::UNAUTHORIZED)]
#[case::reader_token_lacks_write(
    "/+trash?repository=private",
    Some(("external", READER_SECRET)),
    StatusCode::FORBIDDEN
)]
#[case::unknown_repository_token("/+trash?repository=missing", Some(("external", ADMIN_SECRET)), StatusCode::UNAUTHORIZED)]
#[case::invalid_token("/+trash?repository=private", Some(("external", "wrong-secret")), StatusCode::UNAUTHORIZED)]
...
async fn test_trash_record_rejects_an_invalid_ecosystem() {
    ...
    let (status, _, document) = get(
        &app,
        "/+trash/record?ecosystem=%20&repository=private&resource=demo",
        Some(("Alice", USER_PASSWORD)),
    )
    .await;
...
async fn test_trash_accepts_unregistered_ecosystems() {
    ...
    let (inspect_status, _, _) = get(
        &app,
        "/+trash/record?ecosystem=unregistered&repository=private&resource=artifact-a",
        Some(("Alice", USER_PASSWORD)),
    )
    .await;
