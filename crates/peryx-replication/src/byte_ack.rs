...
/// The decision to acknowledge a client write from the durability evidence gathered so far.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ByteAckDecision {
    /// Enough independent nodes proved durability; acknowledge the write, naming the nodes that proved
    /// it.
    Acknowledged { nodes: Vec<String> },
    /// The bytes are not yet durable; `remaining` more independent receipts are needed. The nodes that
    /// have already acknowledged are preserved so a retry resumes from them.
    Pending { nodes: Vec<String>, remaining: usize },
}

impl ByteAckDecision {
    /// Whether the write is datacenter-acknowledged.
    #[must_use]
