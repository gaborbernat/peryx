...
/// How fully a query's answer covers its expected producers.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Completeness {
    /// Every expected producer's accepted frontier reaches its required position.
    Complete,
    /// Every expected producer has an accepted frontier, but at least one trails its required position,
    /// so the missing totals should still arrive.
    Delayed,
    /// An expected producer has no accepted frontier, so the answer's coverage cannot be vouched for.
    Unavailable,
}
