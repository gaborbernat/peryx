...
/// A same-datacenter peer's proof that it durably holds an artifact's bytes: the node that answered and
/// the digest and byte length its filesystem placement receipt verified.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PeerReceipt {
    /// The independent configured node whose store answered.
    pub node: String,
    /// The content address the peer proved durable.
    pub digest: Digest,
    /// The durable byte length the peer reported for the digest.
    pub size: u64,
}

impl PeerReceipt {
    /// The quorum evidence this receipt contributes: the peer and the digest it proved. The byte length
    /// is transport fidelity, not a term the quorum reads.
    #[must_use]
    pub fn into_ack(self) -> ReceiptAck {
        ReceiptAck {
            node: self.node,
            digest: self.digest,
        }
...
/// One same-datacenter peer a write queries for its placement receipt of a digest.
///
/// Absence and transport loss are both `Ok(None)`/`Err` rather than a failed write: a peer that has not
/// replicated the bytes yet, or that a fault hides this round, is re-queried while the deadline is live.
#[async_trait]
pub trait ReceiptSource: Sync {
    /// The configured node identity this source speaks for, so the gather skips a peer it already holds a
    /// receipt from and attributes the evidence to the right member.
    fn node(&self) -> &str;

    /// Ask the peer whether it durably holds `digest`, returning its receipt or `None` when it does not
    /// yet.
    ///
    /// # Errors
    /// Returns a [`TransportError`] on transport loss or a peer-side rejection; the gather treats it as a
    /// peer that did not contribute this round rather than a failed write.
