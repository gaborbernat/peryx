...
/// How many members must durably hold a serial before the group guarantees it.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DurabilityPolicy {
    /// Acknowledge from the writer's local durability alone: one member must hold the serial.
    Local,
    /// A serial is durable once a strict majority of the configured members hold it.
    Majority,
    /// A serial is durable once every configured member holds it.
    Everywhere,
    /// A serial is durable once at least this many configured members hold it.
    AtLeast(NonZeroUsize),
}

impl DurabilityPolicy {
    /// The number of members that must hold a serial for it to be durable in a group of `configured`
    /// members. [`Majority`](Self::Majority) rounds up past half, so a two-member group still needs both
    /// and a three-member group needs two. [`AtLeast`](Self::AtLeast) may exceed the group, in which case
    /// no serial can ever satisfy it.
    #[must_use]
