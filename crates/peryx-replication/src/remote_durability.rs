...
/// One remote datacenter's acknowledgement: how far it has durably applied, under which authority epoch.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RemoteAck {
    pub datacenter: String,
    /// The authority epoch the remote applied under; only the operation's own epoch is eligible.
    pub epoch: u64,
    /// The highest apply frontier the remote has made durable.
    pub applied_frontier: u64,
}

/// The metadata operation a write waits to see remote-durable: the authority epoch it committed under
/// and the apply frontier that covers it.
