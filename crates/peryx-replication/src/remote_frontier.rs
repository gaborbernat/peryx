...
/// One eligible remote datacenter a write queries for its metadata apply-frontier.
///
/// Absence, a not-yet-covered frontier, and transport loss are all a peer that did not contribute this
/// round rather than a failed write: the remote is re-queried while the deadline is live.
#[async_trait]
pub trait RemoteFrontierSource: Sync {
    /// The remote datacenter this source speaks for, so the gather keeps only that datacenter's latest
    /// acknowledgement and attributes the evidence to the right holder.
    fn datacenter(&self) -> &str;

    /// Ask the remote how far it has durably applied `authority`'s journal and the authority epoch it
    /// applied under, or `None` when it is not currently reporting one. The query is authority-scoped
    /// because the epoch a write commits under is the authority's own, not a group-wide term.
    ///
    /// # Errors
    /// Returns a [`TransportError`] on transport loss or a remote-side rejection; the gather treats it as
    /// a remote that did not contribute this round rather than a failed write.
