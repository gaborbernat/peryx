...
fn defect_turns_into_its_matching_invariant_failure() {
    ...
    for (defect, plan, invariant) in cases {
        ...
        let Outcome::Violated { invariant: broken, .. } = trace.outcome else {
            panic!("{defect:?} did not break {invariant:?}: {trace:?}");
        };
        assert_eq!(broken, invariant, "{defect:?} broke the wrong invariant");
    }
}

#[test]
fn a_correct_model_survives_each_defect_plan_without_the_defect() {
