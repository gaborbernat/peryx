...
fn test_revoke_blocks_the_next_verification_and_is_idempotent() {
    ...
    let RevokeScopedTokenOutcome::Revoked(revoked) = store.revoke_scoped_token(&created.id, 300).unwrap().unwrap()
    else {
        panic!("first revoke should transition the token");
    };
...
fn test_list_paginates_within_a_reach() {
    ...
    let second = store
        .list_scoped_tokens(&ScopedTokenQuery {
            reach: repo("hosted"),
            cursor: Some(TokenId::new(cursor)),
            limit: 2,
        })
        .unwrap();
    assert_eq!(second.tokens.len(), 1);
    assert_eq!(second.next_cursor, None);
    assert!(second.tokens.iter().all(|token| token.reach == repo("hosted")));
}
...
#[test]
fn test_list_rejects_an_out_of_range_limit() {
    let (_dir, store) = store();
    for limit in [0, 101] {
        let error = store
            .list_scoped_tokens(&ScopedTokenQuery {
