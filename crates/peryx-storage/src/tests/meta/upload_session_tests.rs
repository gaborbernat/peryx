...
use crate::meta::{
    AccountingClass, MetaError, MetaStore, NewQuotaReservation, QuotaError, QuotaLimits, QuotaValue, UploadRecord,
};
...
fn membership(txn: &mut crate::meta::DriverTxn) -> Result<((), Vec<Vec<u8>>), MetaError> {
    txn.put(MEMBERSHIP, &[])?;
    Ok(((), Vec::new()))
}

fn membership_fault(_txn: &mut crate::meta::DriverTxn) -> Result<((), Vec<Vec<u8>>), MetaError> {
...
fn test_closing_upload_keeps_the_session_when_membership_fails() {
    ...
    // The session survives, so the client's recovery handle still resolves for a retry.
...
fn test_metered_closing_upload_keeps_the_session_when_membership_fails() {
    ...
    assert!(store.upload_record("session-1").unwrap().is_some());
    assert_eq!(store.get_driver_value(MEMBERSHIP).unwrap(), None);
    // The reservation stays pending for the retry rather than committing against a lost membership.
    assert_eq!(
        store.quota_usage("store").unwrap().accounted_bytes,
        QuotaValue {
            committed: 0,
