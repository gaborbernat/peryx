...
use crate::config::{Config, DcMembership};
...
impl FilesystemPlacementReconciler {
    ...
    fn plan(membership: &DcMembership, identity: &str, store: BlobStore) -> anyhow::Result<Option<Self>> {
        ...
        let mut target_dcs = BTreeSet::new();
        for member in &membership.members {
            let dc = DataCenterId::new(&member.dc)
                .with_context(|| format!("datacenter {:?} is not a valid placement component", member.dc))?;
            target_dcs.insert(dc);
        }
        // A single distinct data center has no peer to repair a rotted or missing copy from and no
        // out-of-policy data center to retire, so it reconciles nothing.
        if target_dcs.len() < 2 {
            return Ok(None);
        }
        Ok(Some(Self {
...
#[cfg(test)]
#[path = "placement_reconcile_tests.rs"]
mod tests;
