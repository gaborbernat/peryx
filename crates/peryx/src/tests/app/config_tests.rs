...
/// A blob backend that proves replicated durability, so `dc` and `ha` configurations pass the
/// cross-field durability check that `config check` runs before it summarizes the mode.
