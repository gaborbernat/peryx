...
Repeat `mise install --locked` when a checkout changes `mise.toml` or `mise.lock`. Run `rustup show` when the compiler
version changes; it downloads the version pinned by `rust-toolchain.toml`. A linker or system-library error means the
host lacks a build dependency for its target. Check the failing package name and install that dependency through the
