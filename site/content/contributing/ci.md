...
list also holds mutants that are the original under another spelling, such as `Poll::from(None)` in `Ended::poll_frame`,
where `From<T> for Poll<T>` is `Poll::Ready`. An entry of that kind names the single replacement and states the proof
beside it, and only when the code has no simpler spelling that removes the mutant. After changing the list, diff
