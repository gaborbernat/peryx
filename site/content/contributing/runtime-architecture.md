...
description = "How the binary composes ecosystem owners, shared services, storage, and availability."
weight = 2
+++
...
set of indexes owned by the same ecosystem. The [index model](@/core/repositories/indexes.md) defines shared rules. The
...
`AvailabilityRuntime::prepare` returns a `PreparedAvailability` containing bound listeners, routes, metrics, and worker
resources without starting managed work. The process mounts the returned routes and finishes its startup checks before
`PreparedAvailability::activate` transfers resource ownership to `ActiveAvailability`. Activation starts consensus,
listeners, replication, and reconciliation as one transition. A failed transition stops each resource that started.

The distinct prepared and active types prevent double activation and waits on resources that have not started. The
process holds the active handle until shutdown. It cancels work first, then joins listeners, consensus, and workers
within a bounded deadline. A resource that misses the deadline moves to the process reaper, which waits outside the
serving runtime. Dropping either handle requests cancellation without blocking the dropping thread.

`peryx-ha` owns lifecycle, placement, reclamation, frontier, operation, and persistence traits. Storage implements
atomic operations without choosing policy. `peryx-ha-distributed` owns consensus, replication, cross-datacenter copy,
reconciliation, reclamation, authority transfer, metrics, and tasks. Ecosystem owners expose reference and operation
