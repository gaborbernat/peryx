...
description = "How an operator deliberately moves a healthy authority's home to another datacenter, gated on the target catching up, and how the move records who did it and why."
weight = 8
+++
...
migration. Because the old home is still serving, the move waits for the target to catch up before it commits, so the
new home takes ownership holding every write the old home acknowledged. This page covers the request and cancel
endpoints, the catch-up gate, the states a transfer moves through, cancellation, the audit it records, and recovery. It
runs on the [availability control listener](@/core/availability-listener.md) and mints the same fencing epoch the
[availability contracts](@/core/availability-contracts.md) describe.
...
stateDiagram-v2
[*] --> AwaitingCatchUp
AwaitingCatchUp --> Ready: target reaches the barrier
Ready --> Committed: commit mints the epoch and records the audit
AwaitingCatchUp --> Cancelled: operator cancels
...
To move a live home in an `ha` or multi-datacenter `dc` deployment:
