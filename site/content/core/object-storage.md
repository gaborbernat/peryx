...
[monitoring guide](@/core/monitor.md#check-operational-status) covers the rest of that document.
...
- Backups omit the objects. `peryx backup` snapshots the `[blob]` selection without credentials so a restore points at
  ...
  bucket for blob-level recovery, and see [backup and restore](@/core/backup-restore.md) for the metadata half.

The [`[blob]` reference](@/core/configuration.md#blob) lists every key, its default, and the full durability model.
