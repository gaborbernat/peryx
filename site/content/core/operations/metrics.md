...
description = "Scrape behavior, label limits, counting rules, and metric registration."
weight = 8
+++

`GET /metrics` returns Prometheus text exposition with content type `text/plain; version=0.0.4`. Counters and gauges
live in memory and reset on restart. A scraper provides durable time series.
...
An ecosystem owner registers each metric family with its supported roles and label set. Registration rejects undeclared
...
Shared examples use `<registered-ecosystem-id>` in label selectors. Ecosystem guides provide concrete selectors.

## Counting rules

Serving counters follow the monitoring API completion rule. A full response increments artifact count and bytes after
...
Shared collectors cover the process and storage. Each ecosystem exposes serving, metadata, publication, quota, catalog,
and protocol metric families. Its guide lists exact series names, roles, and alert queries.
...
- [Monitoring APIs](@/core/operations/monitor.md)
- [Availability deployment and sizing](@/core/availability/deployment.md)
- [Availability modes](@/core/availability/high-availability.md)
