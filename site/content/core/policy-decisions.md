...
| `project`, `version`, `filename` | Package subject evaluated by the policy                                 |
| `source`                         | Configured route source name, without an upstream URL or credentials    |
| `action`                         | Operation evaluated, such as serving or publishing an artifact          |
...
Query one repository with its upload token:
...
stop at 512 bytes. Repository, rule, and source query filters use the same 512-byte bound. These limits bound query work
and stored audit data.

Authorization runs before the history query. An upload token can inspect only its repository. Local administrators can
...
matched package facts; they must not include secrets from configuration or requests.

## Troubleshooting

Send local passwords and repository tokens over HTTPS, except for a loopback-only server. Configure Peryx TLS or
terminate TLS at a trusted reverse proxy before exposing this page. Reloading the page clears the credentials held by
the hydrated form.
