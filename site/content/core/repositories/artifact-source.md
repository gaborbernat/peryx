...
peryx records two independent facts about each artifact, apart from admission, visibility, trash, and
[revocation](@/core/repositories/digest-revocations.md). **Source** is where the bytes came from. **Byte availability**
is whether this instance can serve them right now. An artifact read resolves both from one indexed lookup, so a listing
never probes the content store per artifact.

The two dimensions are independent. A proxied artifact may have cached bytes; a hosted artifact is local until its bytes
are lost. Neither says whether admission rules permit the download or whether the owner hides the record, or whether an
administrator revoked the digest. A read applies those dimensions after source and availability.

## Glossary

**Source** is where an artifact's bytes originate. Caching or evicting the bytes does not change it.

- `hosted`: a client sent the artifact to this instance. No upstream can resupply lost bytes.
...
| --------------------------------------------------------------------------- | -------------------------------------------- |
| Recorded with verified local bytes (ingest, generate, completed cache fill) | `local`                                      |
...
- **One indexed lookup.** Source and availability live in one record keyed by content digest. An artifact read resolves
