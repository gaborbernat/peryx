...
[artifact source and availability](@/core/repositories/artifact-source.md) projection reports whether one instance can
serve bytes; the placement ledger records every known location and state.

This contract covers ledger state and routing queries. Transfer, peer streaming, and operator UI contracts are separate.

The placement ledger stores neutral artifact and blob identifiers. Atomic updates protect placement changes while the
distributed coordinator applies transition policy, copying, routing, and reconciliation. Ecosystem-specific metadata
