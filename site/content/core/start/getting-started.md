...
See [installation](@/core/start/installation.md) for Windows and update behavior.
...
Use the [configuration reference](@/core/operations/configuration.md) for shared process, storage, access, job, and
availability keys. `peryx config check --config peryx.toml` rejects unknown owner IDs, unknown availability modes, and
incomplete distributed settings before startup.
