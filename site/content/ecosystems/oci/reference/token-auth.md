...
[Bearer token realm](@/ecosystems/oci/token-realm.md) for the protocol design.
...
- `Basic` credentials: peryx checks the password against every OCI index's tokens. A password that authenticates nowhere
  gets `401`, so `docker login` rejects it. A password that authenticates names its subject. A scope on another index is
  granted only when the same header authenticates as that subject there, so equal token names with different secrets
  remain isolated.
