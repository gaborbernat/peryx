...
description = "Simple API version derivation, gpg-sig retention, provenance, and slashless URL redirects."
weight = 4
aliases = [ "/ecosystems/pypi/reference/api-version/", "/ecosystems/pypi/reference/gpg-sig/", "/ecosystems/pypi/reference/trailing-slash/"]
+++

peryx derives the advertised version from upstream data, retains signature markers only for reachable signatures, and
redirects clients to canonical URLs. See [Simple API serving](@/ecosystems/pypi/serving.md) for the design and
[HTTP endpoints](@/ecosystems/pypi/reference/endpoints.md) for routes.
...
- Serving rationale: [Simple API serving](@/ecosystems/pypi/serving.md)
...
- End-to-end examples: [Simple API behavior](@/ecosystems/pypi/tutorials/simple-api-behavior.md)
